...
file_path = "/home4/s6273475/ml/master_project/data/EGFR_IC50_protein_based.xlsx"
...
# Generate the configuration dictionary for the model without the 'use_cuda' parameter.
...
# Set GPU usage in the config dictionary.
...
# (Assumes model.train returns a dictionary with loss histories)
# -------------------------------
try:
    ...
    if "no kernel image is available" in str(e):
        ...
        # Disable GPU by clearing visible devices.
        os.environ["CUDA_VISIBLE_DEVICES"] = ""
        config["use_cuda"] = False
        # Reinitialize the model for CPU-only execution.
        model = models.model_initialize(**config)
        history = model.train(train, val, test)
    else:
        raise

# If the training history is not returned, you might need to manually track losses.
# Here we assume history is a dict with keys 'train_losses' and 'val_losses'
...
model_save_path = "/home4/s6273475/ml/master_project/models/cnn_cnn/trained_model.pth"
...
def predict_with_uncertainty(model, data, n_iter=10):
    ...
    """
    # Activate dropout layers in the underlying PyTorch module.
    # Replace 'model.model' with the appropriate attribute if different.
    model.model.train()
...
if train_losses and val_losses:
    ...
    loss_plot_path = os.path.join(
        "/home4/s6273475/ml/master_project/models/cnn_cnn/cnn_cnn",
...
val_plot_path = os.path.join(
    "/home4/s6273475/ml/master_project/models/cnn_cnn",
...
test_plot_path = os.path.join(
    "/home4/s6273475/ml/master_project/models/cnn_cnn", "test_inference_results.png"
)
...
output_dir = "/home4/s6273475/ml/master_project/models/cnn_cnn"
...
# Print out summary metrics for both validation and test sets
print("\nValidation Results:")
